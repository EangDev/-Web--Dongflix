...
export default function RecommendedPage() {
    ...
    useEffect(() => {
        const fetchRecommended = async () => {
            try{
                const res = await fetch("http://127.0.0.1:8000/api/anime/recommended");
                const json = await res.json();
                setRecommended(json.data || []);
            }catch(error){
                console.error("Error fetching Recommended Series API", error);
            }finally{
                setLoading(false);
            }
        };
                ...
                <div className="relative z-10">
                    ...
                    <div className="w-[90%] mx-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-12">
                        {recommended.map((item, idx) => (
                            <div key={idx} className="relative flex flex-col items-center cursor-pointer group">
