...
export default function LatestPage(){
    ...
    const [latestDong, setLatestDong] = useState([]);
    ...
    useEffect(() => {
        const fetchLatest = async () => {
            try{
                const res = await fetch("http://127.0.0.1:8000/api/anime/latest");
                const json = await res.json();
                setLatestDong(json.data || []);
            }catch(error){
                console.error("Error fetching Latest Page API", error);
            }finally{
                setLoading(false);
            }
        };
    ...
    const currentItems = viewAll ? latestDong : latestDong.slice(indexOfFirst, indexOfLast);
    const totalPage = Math.ceil(latestDong.length / ITEM_PER_PAGE);
    ...
    const handleDetail = (item) => {
      if(!item.link){
        alert("No available url for this anime!");
        return;
      }
    ...
    return(
        <>
            ...
            <section className="relative min-h-screen mx-auto pt-15 max-w-7xl bg-linear-to-b from-[#050505] via-[#111] to-[#0a0a0a] rounded-2xl overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <div className="flex justify-center mt-7.5 gap-3 pb-10">
                        {!viewAll ? (
                          <>
                            <button
                              onClick={handlePrev}
